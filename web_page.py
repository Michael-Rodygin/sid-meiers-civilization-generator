...
def create_web_page(df1, df2, df3, df4, df5, df6, initial_players):
    ...
    tab0, tab1, tab2 = st.tabs(["📜 Сюжет", "🏛️ Цивилизации", "🗺️ Карта Территории"])

    with tab0:
        st.markdown('<div class="lore-title">Летопись Острова</div>', unsafe_allow_html=True)
        st.markdown('<div class="lore-subtitle">Штормы времени раздвинули эпохи, и правители, ведомые амбициями, одновременно ступили на землю, где история рождается заново.</div>', unsafe_allow_html=True)
        general = "В мире существует Архипелаг Времён — кластер островов, затерянных не только в пространстве, но и во времени. Его местоположение скрыто Хронозавесой, мистической аномалией, созданной древней цивилизацией-хранителем, пытавшейся уберечь человечество от самоуничтожения. Когда Земля достигает точки критического напряжения - будь то ядерная угроза, экологический коллапс или глобальный конфликт - Хронозавеса активируется и призывает из разных эпох величайших стратегов, реформаторов и завоевателей, перенося их в «петлю времени» на архипелаге. Им даётся одно задание - найти путь, который приведет человечество к процветанию, объединив все народы, будь то через войну, культуру, технологии или экономику. Тот, чья философия окажется наиболее жизнеспособной, не только вернётся в свою эпоху с обновлённым знанием, но и изменит ход мировой истории. Однако Хронозавеса не выбирает слабых - только тех, чьи имена навсегда вписаны в летописи величия."
        lore_html = '<div class="lore-container">'
        lore_html += f'<div class="lore-card"><div class="lore-text">{general}</div></div>'
        title_map = {
            'Египтяне': 'Фараон древнего Египта',
            'Римляне': 'Римский Император',
            'Русские': 'Царь Российской Империи',
            'Китайцы': 'Хуанди Китайской Империи',
            'Ацтеки': 'Тлатоани Ацтеков',
            'Англичане': 'Король Британской Империи',
            'Испанцы': 'Король Испанской Империи',
            'Монголы': 'Хан Монголов',
            'Американцы': 'Президент США',
            'Французы': 'Первый Консул Французской Республики',
            'Немцы': 'Канцлер Объединённой Германии',
            'Арабы': 'Арабский Султан',
            'Греки': 'Архонт Древней Греции',
            'Индийцы': 'Махараджа Индийской Империи',
            'Японцы': 'Японский Сёгун',
            'Зулусы': 'Инкоси Зулусов',
        }
        reason_map = {
            'Египтяне': '«Вечность строится из камня и песка». Фараон ищет новую долину, где воздвигнуть пирамиды и закрепить имя в вечности, заключив союз с богами через камень и песок.',
            'Римляне': '«Закон — величайшее оружие империи». Желает расширить Римский Мир через неизведанные воды, принести закон, дороги и величие своей цивилизации на новый берег.',
            'Русские': '«Сквозь мороз и пламя — к морю и славе». Стремится к славе и укреплению державы, привёз ледяную стойкость и огонь реформ, чтобы доказать право на море.',
            'Китайцы': '«Дао — путь к гармонии». Следует Небесному Мандату, ищет гармонию стихий и доказательство, что добродетель управляет даже за пределами Поднебесной.',
            'Ацтеки': '«Барабаны зовут — боги ждут сердца». Ищет священную землю для храмов и обрядов, чтобы сердца богов снова услышали песнь жрецов и звук барабанов.',
            'Англичане': '«Империя, над которой никогда не заходит солнце». Ведёт флот и порядок, мечтая о портах, колониях и законах в каждом уголке мира.',
            'Испанцы': '«За веру, золото и корону!». Гонимый верой и золотом, стремится поднять крест и знамёна Кастилии над новым Эльдорадо.',
            'Монголы': '«Где ступит конь — там будет ханство». Ветер степей привёл его конных воинов к океану; теперь он ищет простор для стремительного удара и новой ставки ханства.',
            'Американцы': '«Американская мечта: свобода и возможности для каждого». Стремится к свободе и прогрессу, строит рынок идей и технологий, чтобы остров стал кузницей будущего.',
            'Французы': '«Разум, слава и право — выше тронов». Принёс разум эпохи революции, мечтает узаконить новые порядки и увенчать остров сиянием республики.',
            'Немцы': '«Порядок — основа величия». Ищет порядок и индустрию, чтобы превратить остров в механическое чудо дисциплины и изобретательности.',
            'Арабы': '«Торговля — мост между мирами». Прибыл с мудростью пустыни и мастерством торговли, желая превратить перекрёсток путей в сердце караванов.',
            'Греки': '«Доблесть — в слове и в мече». Мечтает о полисе, где философы и воины равны перед доблестью, а море служит триерам и музам.',
            'Индийцы': '«Дхарма ведёт к процветанию». Привёз пряности, мудрецов и колесо дхармы, чтобы обрести равновесие и процветание на новой земле.',
            'Японцы': '«Сталь и честь — едины». Следует пути бусидо, ищет островную дисциплину и кузницы, где сталь и честь правят совместно.',
            'Зулусы': '«Сила — в единстве и ритме предков». Несёт дух предков и ритм барабана, собирает отряды, чтобы вернуть земле силу и гордость народа.',
        }
        for i, player in enumerate(players):
            if i in tables:
                try:
                    nation = tables[i].loc['Нация: ', 'bonus']
                except Exception:
                    nation = ''
                if str(player).strip() and str(nation).strip():
                    title = title_map.get(nation, 'Правитель')
                    reason = reason_map.get(nation, 'Прибыл, чтобы испытать судьбу и вписать своё имя в хроники острова.')
                    lore_html += f'<div class="lore-card"><h4>{title} {player}</h4><div class="lore-text">{reason}</div></div>'
        lore_html += '</div>'
